...
with pestanas[0]:
    ...
    if columnas_outliers:
        ...
        st.dataframe(o.resumen_outliers(datos, columnas_outliers), use_container_width=True, hide_index=True)
        ...
        st.dataframe(datos.loc[mascara, columnas_outliers].head(50), use_container_width=True)
...
with pestanas[1]:
    ...
    if len(columnas_cluster) < 2:
        ...
        if resultado is not None and resultado.columnas == columnas_cluster:
            ...
            st.bar_chart(con_clusters["cluster"].value_counts(dropna=False).sort_index())
